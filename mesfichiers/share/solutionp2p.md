# Yabisso - Code du partage automatique P2P (V3.9)


Genere le 2026-06-05 03:04:47
...
- `P2PAutoSync.js` - Orchestrateur principal - synchronisation P2P bidirectionnelle (V3.9)
...
- Conservation du double-validateur PACK_RECEIVED_OK (V3.6.4)
...
_Orchestrateur principal - synchronisation P2P bidirectionnelle (V3.9)_

- **Chemin source** : `C:\Users\Utilisateur\Documents\Ben\myapp\yabisso\app\src\features\bluetooth\services\P2PAutoSync.js`
- **Lignes** : 1259
- **Caracteres** : 57619
...
class P2PAutoSyncClass {
  ...
  async _waitForSlaveConfirmation(peerName, timeoutMs = 20000) {
  ...
  async start() {
    ...
    this._log('🚀 Orchestrateur démarré (V3.9 - BARRIÈRE _waitingForSlave + .then() pattern, reset handshake à chaque session).');
    ...
    try {
      ...
      WifiDirectService.on('onConnectionChange', async ({ connected, info }) => {
        if (connected && this._running) {
          ...
          // - Master : startReceiving + _waitingForSlave=true + _waitForSlaveConfirmation
          //   en .then() (PAS d'await dans setTimeout). Le cycle 3s est bloqué par
          //   la barrière _waitingForSlave tant que le Slave n'a pas confirmé.
          // - Slave : startReceiving + _sendYabissoHello immédiat.
          setTimeout(() => {
            if (!WifiDirectService.connectedPeer || !this._running) return;
            if (isMyRoleMaster) {
              this._log(`■ [V3.9] Master : récepteur démarré, verrou ON`);
              ...
              this._waitForSlaveConfirmation(peerName, 20000).then(confirmed => {
                ...
                if (confirmed && this._running && WifiDirectService.connectedPeer) {
                  this._log(`■ [V3.9] ✅ Slave confirmé → lancement envoi`);
                  this._p2pSyncCycle();
                } else {
                  this._log(`■ [V3.9] ⏰ Timeout — abandon`);
                }
              });
            } else {
              this._log(`■ [V3.9] Slave : récepteur + HELLO`);
              WifiDirectService.startReceiving(WifiDirectService.globalFileHandler);
...
_Service Mesh BLE - decouverte et election des roles_

- **Chemin source** : `C:\Users\Utilisateur\Documents\Ben\myapp\yabisso\app\src\features\bluetooth\services\NearbyMeshService.js`
- **Lignes** : 406
- **Caracteres** : 15679
...
_Service WiFi Direct - transport de fichiers_

- **Chemin source** : `C:\Users\Utilisateur\Documents\Ben\myapp\yabisso\app\src\features\bluetooth\services\WifiDirectService.js`
- **Lignes** : 605
- **Caracteres** : 22127
